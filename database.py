...
class DbInsert(tornado.web.RequestHandler):
	...
	def insert_in_zsets(self,arr,key):
		...
		for i in range (len(arr)):
			r.zadd("search_suggest:prefix:"+str(arr[i]),0,key)
...
class DbSearch(tornado.web.RequestHandler):
	def post(self):
		...
		for i in range(prefix_array_length):
		 	r.zinterstore("dest",["search_suggest:prefix:"+str(prefix_array[0]),"search_suggest:prefix:"+str(prefix_array[i])])
		key_array=r.zrange("dest",0,-1)
		json_return=r.hmget('search_suggest:terms',key_array)
